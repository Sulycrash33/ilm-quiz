...
export function DailyProgressCard({ questionsToday, accuracy }: DailyProgressCardProps) {
  ...
  return <Card className="border-2 border-primary/20 shadow-lg h-full"><CardHeader><CardTitle className="flex items-center gap-2 font-headline text-primary"><Target className="h-5 w-5" aria-hidden="true" />{t("todaysMission")} <Sparkles className="h-4 w-4 ml-auto text-accent" /></CardTitle></CardHeader><CardContent className="space-y-5"><div><div className="flex items-end justify-between"><div><div className="text-3xl font-bold text-primary tabular-nums">{questionsToday}/{REWARD_RULES.dailyMissionQuestions}</div><div className="text-sm text-muted-foreground">{t("questionsAnsweredLower")}</div></div><div className="text-right"><div className="text-2xl font-bold text-emerald-400 tabular-nums">{accuracy === null ? '—' : `${accuracy}%`}</div><div className="text-xs text-muted-foreground">{t("todaysAccuracy")}</div></div></div><Progress value={dailyProgress} className="mt-4 h-2" aria-label={`Daily mission progress: ${dailyProgress}%`} /></div><div className={`rounded-lg p-3 text-sm ${complete ? 'bg-emerald-400/10 text-emerald-400' : 'bg-secondary text-secondary-foreground'}`}><strong>{complete ? t("missionCompleteLabel") : t("nextMove")}</strong><div>{complete ? t("dailyMissionComplete") : t("questionsToFinishMission", { count: REWARD_RULES.dailyMissionQuestions - questionsToday })}</div></div></CardContent></Card>;
}
