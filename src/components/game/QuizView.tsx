...
const LIFELINES: Lifeline[] = [
  { id: 'fifty-fifty', name: '50/50', icon: 'âš¡', cost: 50, description: 'Remove two wrong answers' },
  { id: 'ask-imam', name: 'Ask Imam', icon: 'ðŸ§ ', cost: 75, description: 'Get a helpful hint' },
  { id: 'skip', name: 'Skip', icon: 'â­ï¸', cost: 25, description: 'Skip to next question' },
  { id: 'double-points', name: '2x Points', icon: 'ðŸ’Ž', cost: 100, description: 'Double points for this question' },
  { id: 'time-boost', name: 'Time+', icon: 'â°', cost: 30, description: 'Add 15 seconds' },
];
...
export function QuizView({ questions, categoryTitle, onExit }: QuizViewProps) {
  ...
  if (questions.length === 0) return <div className="flex flex-col items-center justify-center h-full text-center p-8"><h2 className="text-2xl font-bold mb-2">Coming Soon!</h2><p className="text-muted-foreground">Questions for the â€œ{categoryTitle}â€ category are being prepared.</p><Button onClick={onExit} className="mt-6">Back to Categories</Button></div>;
  if (showResult) return <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="container mx-auto px-4 py-6 max-w-2xl"><Card className="border-2 border-primary/20 shadow-xl"><CardHeader className="text-center"><motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-6xl mb-4">ðŸ†</motion.div><CardTitle className="text-2xl text-primary">Round complete</CardTitle></CardHeader><CardContent className="pt-6 space-y-6"><div className="grid grid-cols-3 gap-4 text-center"><div className="bg-secondary p-4 rounded-lg"><div className="text-2xl font-bold text-primary">{score}</div><div className="text-sm text-muted-foreground">XP this round</div></div><div className="bg-accent/10 p-4 rounded-lg"><div className="text-2xl font-bold text-accent">{coins}</div><div className="text-sm text-muted-foreground">Coins</div></div><div className="bg-primary/10 p-4 rounded-lg"><div className="text-2xl font-bold text-primary">{highScore}</div><div className="text-sm text-primary/80">Best total</div></div></div><div className="flex gap-4"><Button onClick={restartQuiz} className="flex-1">Play again</Button><Button variant="outline" className="flex-1" onClick={onExit}>Back to categories</Button></div></CardContent></Card></motion.div>;
